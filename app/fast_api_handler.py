...
class FastApiHandler:
    ...
    def churn_predict(self, model_params: dict) -> float:
        ...
        """
        order_params = ['gender', 'SeniorCitizen', 'Partner',
        ...
        order_params_dict = OrderedDict((key, model_params[key]) for key in order_params)
        params_value_list = list(order_params_dict.values())
        return self.model.predict_proba(params_value_list)[1]
        ...
        try:
            if not self.validate_params(params):
                ...
                response = {
                    ...
                    'is_churn': [0, 1][int(probability > 0.5)]
                }

        except Exception as e:
            return {"Error": f"Problem with request: {str(e)}"}
        else:
            return response
